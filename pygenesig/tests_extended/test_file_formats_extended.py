...
class TestFileFormats(unittest.TestCase):
    def test_gct(self):
        exprs = read_expr("/pstore/data/bioinfo/users/sturmg/gtex-signatures/data_processed/v3/exprs_processed.npy")
        target = read_target("/pstore/data/bioinfo/users/sturmg/gtex-signatures/data_processed/v3/target.csv")
        # read target used on purpose, as we don't want that in a dict, but an array
        rosetta = read_rosetta("/pstore/data/bioinfo/users/sturmg/gtex-signatures/data_processed/v3/rosetta_processed.csv", as_dict=False)
