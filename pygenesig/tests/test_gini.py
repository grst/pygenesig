...
class TestGini(unittest.TestCase):
    ...
    def test_get_gini_format(self):
        """Check that the rogini output generated by pygenesig matches the 'original' by Laura. """
        df_aggr = pd.read_csv("./gini/gini_test.tsv", index_col=0, sep="\t")
        rogini_res = pd.read_csv("./gini/gini_test_0-5.result.txt", sep="\t")
        rogini_res = rogini_res.iloc[:, :5]  # skip last column, as not complementary.

        gini_format = get_rogini_format(df_aggr, min_gini=.5, max_rk=1, min_expr=0)
        pdt.assert_almost_equal(rogini_res, gini_format)

    def test_get_gini_format(self):
        """Check that the rogini output generated by pygenesig matches the 'original' by Laura. """
        df_aggr = pd.read_csv("./gini/gini_test_rank.tsv", index_col=0, sep="\t")
        rogini_res = pd.read_csv("./gini/gini_test_rank.result.txt", sep="\t")
